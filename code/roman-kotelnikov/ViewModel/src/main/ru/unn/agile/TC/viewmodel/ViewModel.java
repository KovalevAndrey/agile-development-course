...
public class ViewModel {
    ...
    public void convert() {
        if (inputScale == null) {
            ...
            logger.puts(LOG_ERROR_INPUT_SCALE_IS_NULL);
            return;
        }

        if (resultScale == null) {
            status = Status.STATUS_RESULT_SCALE_NULL;
            logger.puts(LOG_ERROR_RESULT_SCALE_IS_NULL);
            return;
        }
        ...
        try {
            ...
            logger.puts(LOG_ERROR_WRONG_INPUT_STRING);
            return;
        }

        status = Status.STATUS_VIEW_MODEL_OK;
        logger.putsString(LOG_VIEW_MODEL_OK);
    }
